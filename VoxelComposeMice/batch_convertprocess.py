...
"""
批量逆向处理工具
对文件夹下的所有相位NIFTI文件进行逆向处理，恢复到原始HU空间
"""
...
# 导入原始的ReverseConversionPipeline类
from convertprocess import ReverseConversionPipeline


class BatchReverseProcessor:
    """批量逆向处理器"""
    
    def __init__(self, input_dir: str, output_dir: str, stats_path: str, 
                 target_range: Tuple[float, float], restore_full_volume: bool = False):
        """
        初始化批量处理器
        ...
        Args:
            ...
            output_dir: 输出目录
            stats_path: HU调整统计文件路径
            target_range: 归一化目标范围
            restore_full_volume: 是否恢复到完整体积
        """
        self.input_dir = Path(input_dir)
        self.output_dir = Path(output_dir)
        self.stats_path = stats_path
        self.target_range = target_range
        self.restore_full_volume = restore_full_volume
        
        # 创建输出目录
        self.output_dir.mkdir(exist_ok=True, parents=True)
        
        # 创建处理管道
        self.pipeline = ReverseConversionPipeline(stats_path, target_range)
    ...
    def process_single_file(self, input_path: Path) -> Path:
        ...
        # 添加后缀标识已恢复
        output_filename = f"{output_filename}_restored.nii.gz"
        ...
        self.pipeline.process(
            str(input_path),
            str(output_path),
            restore_full_volume=self.restore_full_volume
        )
    ...
    def process_all(self, pattern: str = "*.nii.gz") -> List[Path]:
        ...
        # 显示处理配置
        print(f"\n处理配置:")
        print(f"  统计文件: {self.stats_path}")
        print(f"  目标范围: {self.target_range}")
        print(f"  恢复完整体积: {self.restore_full_volume}")
    ...
    def generate_summary_report(self, output_files: List[Path]):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("批量NIFTI逆向处理汇总报告\n")
            f.write("="*70 + "\n\n")
            
            f.write("处理配置:\n")
            f.write(f"  输入目录: {self.input_dir}\n")
            f.write(f"  输出目录: {self.output_dir}\n")
            f.write(f"  统计文件: {self.stats_path}\n")
            f.write(f"  目标范围: {self.target_range}\n")
            f.write(f"  恢复完整体积: {self.restore_full_volume}\n")
            f.write("\n")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="批量逆向处理多个相位的NIFTI文件",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
  # 处理目录下所有.nii.gz文件
  python batch_convertprocess.py \\
      -i VoxelComposeMice/4DCT_NIFIT_split \\
      -o VoxelComposeMice/4DCT_NIFIT_restored \\
      -s VoxelComposeMice/nifti_output/hu_adjustment_stats.yaml
  
  # 只处理特定模式的文件（例如只处理phase开头的文件）
  python batch_convertprocess.py \\
      -i VoxelComposeMice/4DCT_NIFIT_split \\
      -o VoxelComposeMice/4DCT_NIFIT_restored \\
      -s VoxelComposeMice/nifti_output/hu_adjustment_stats.yaml \\
  ...
  python batch_convertprocess.py \\
    ...
    parser.add_argument(
        ...
        metavar=("MIN", "MAX"),
        help="正向归一化时使用的目标范围（默认: -1000 1000）"
    )
    parser.add_argument(
        "--pattern",
        default="*.nii.gz",
...
def main():
    ...
    if not os.path.exists(args.stats):
        print(f"错误: 统计文件不存在: {args.stats}")
        return
    
    try:
        # 创建批量处理器
        processor = BatchReverseProcessor(
            input_dir=args.input_dir,
            output_dir=args.output_dir,
            stats_path=args.stats,
            target_range=tuple(args.target_range),
            restore_full_volume=args.restore_full_volume
        )
        
        # 批量处理
        output_files = processor.process_all(pattern=args.pattern)
        
        # 生成汇总报告
        if not args.no_report and output_files:
            processor.generate_summary_report(output_files)
